...
//Patch by QNAP: Add retry mechanism to init SATA host
    int retry = 0;
...
//Patch by QNAP: Add retry mechanism to init SATA host
    ...
    do 
    {
        ...
        else if(rc == 2)
        {
            //COMINIT state reached but COMWAKE and PHY ready is not reached
            //It is observred that by re-intializing the controller is able to reach COMWAKE/PHY ready state
            printf("PHY ready not detected. Re-initializing again..%d\n",retry);
            mdelay(1);
        }
        else
        {
            printf("System Error: error = %d.\n",rc);
            goto err_out;
        }
    } while(++retry < 100);
    if (retry == 100)
