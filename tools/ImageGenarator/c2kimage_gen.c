...
int main(int argc, char *argv[])
{
	...
	struct stat st;
	char *timestmp, *image, *hash, *signature, *pblcky, *keymode, *base_off, *start_off;
	...
	else if (strcmp(argv[1], "legacy") == 0)
	{
		...
		image = argv[4];
                ...
                printf("base_off = %ld\n", (unsigned long)atoi(base_off));
                printf("start_off = %ld\n", (unsigned long)atoi(start_off));
                stat(image, &st);
                printf("image length = %d\n", (int)st.st_size);

                h0->base_offset = atoi(base_off);
                h0->len         = (int)st.st_size;
                h0->start_offset= atoi(start_off);

		hdr_len = LEGACY_MAX_HEADER_LEN;
	}
	else
	{
		...
		else // Generating public key only
		{
			keylen = bn_atoi(pblcky, &pub_key_buf[0], C2K_KEY_LENGTH_4K);
			if ((keylen * 8) != 1024 && (keylen * 8) != 2048)
                        {
			...
			else
			{
				...
				if((fd1 = open(argv[5], O_RDWR | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH)) == -1)
				{
